...
export function fileExtensionValidator(fileExtension: string): ValidatorFn {
  return (control: AbstractControl): ValidationErrors | null => {
    if(control.value === null) return {fileFormat: "Upload a file"}
    let format = control.value.split(".")
    if(format[1] !== fileExtension) return {fileNotSupported: {value: "File Format Not Supported, only .csv accepted!"}}
    else return  null
  };
}
  ...
  ngOnInit(): void {
    ...
    this.uploadForm = new FormGroup({
      file: new FormControl(null, [
        Validators.required, fileExtensionValidator('csv')
      ]),
    });
  }
